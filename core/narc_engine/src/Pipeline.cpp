...
namespace narc_engine {
    void Pipeline::create(const SwapChain* swapChain, const VkDescriptorSetLayout* m_descriptorSetLayout)
    {
        ...
        if (vkCreateGraphicsPipelines(m_deviceHandler->getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_pipeline) != VK_SUCCESS)
        {
    ...
    {
        vkDestroyPipeline(m_deviceHandler->getDevice(), m_pipeline, nullptr);
        vkDestroyPipelineLayout(m_deviceHandler->getDevice(), m_pipelineLayout, nullptr);
    }
} // narc_engine
